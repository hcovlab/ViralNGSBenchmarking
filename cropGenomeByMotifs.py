...
def cut_edges(motif_pairs:list,sequences:list) -> list:
    ...
    for s_id,sequence in enumerate(sequences):
        try:
            ...
            #cut sequence
            newseq={}
            if endend_id-start_id>8000:
                newseq[list(sequence.keys())[0]]=list(sequence.values())[0][start_id:endend_id]
            else:
                newseq[list(sequence.keys())[0]]=list(sequence.values())[0][start_id:(start_id+8499)]
            sequences_cut.append(newseq)
        except:
            newseq['errorseq']='N'*8500
            sequences_cut.append(newseq)
...
cropped_genomes=cut_edges(motif_pairs=border_motifs,sequences=genomes_nogaps)
